...
下载后将 `.exe` 文件与您的 MP4 视频放在同一文件夹下，双击运行即可。
...
> **注意**：若程序包中包含了 FFmpeg 的可执行文件，需遵守 [FFmpeg 的许可证条款](https://ffmpeg.org/legal.html)（通常为 GPL/LGPL）。FFmpeg 的许可证独立于本项目，请确保在分发时附带 FFmpeg 的源码或相关声明。
